...
swarm as expanding or cohesive which can be used in the analysis of a swarm 
filling a space to identify an exit condition.
\end{abstract}
...
This paper will refer to the scalared resultant magnitude as the \emph{`cohesion/repulsion metric'} of the relationship. The `state' of a swarm is therefore the effect the environmental constraints and algorithms have upon the agent resultant magnitude. The agent's \emph{`cohesion/repulsion metric'} can be considered part of the `quality' measure for a swarm's performance.
