...
\begin{abstract}
  ...
  A widely used metric for the analysis of swarm coordination algorithms is
  based upon the distance between agents. This paper examines this metric and
  introduces an entirely new metric based upon the field effects that createof the cohesion
  and repulsion vectors between agents. In comparing and contrasting the two
  metrics, we find that the \emph{cohesion/repulsion} metric offers a number of
  advantages over the distance metric. In particular, the
  \emph{cohesion/repulsion} metric allows the identification of the essential
  characteristic of a swarm as `expanding', `stable', or `contracting'. This is
  not possible with the distance metric.  Practical swarming applications where
  the new metric is useful include area filling and reconnaissance.
\end{abstract}
...
Many coordination algorithms exist to control the movement of agents within a swarm. The primary goal of these algorithms is to maintain a stable swarm structure. The main focus of this paper is the use any coordination algorithm that are based on \textit{field effects} to produce a metric that allows the swarm to be analysed. To allow this paper to present the findings a basic swarm model is introduced and then the two metrics are applied to this model to present a comparison. This includes the identification of a swarm's `state'. In the remainder of this paper Section~\ref{Section:Related} discusses related work, Section~\ref{Section:SwarmModelling} introduces a basic swarm model for evaluation. Section~\ref{Section:StabilityMagnitude} discusses the mathematical model that produces the swarming effect. Section~\ref{Section::Movement1} discusses how the model is implemented over time to produce movement. Section~\ref{Section:StabilityNullVector} discusses the effect of the model on stability. Section~\ref{Section:Jitter} discusses the effect the mathematical model can have upon the structure of a swarm in terms of potentially wasted energy. Section~\ref{Section:DistanceDynamics} discusses a distance based metric approach to swarm analysis. Section~\ref{Section:MagnitudeDynamics} introduces the new \textit{cohesion/repulsion-based} metric. Section~\ref{Section:ApplicationFloodFilling} compares the two metrics using a scenario of a swarm being used to fill an area. Section~\ref{Section:MagnitudeDistanceComparison} discusses the results of the experiments and Section~\ref{Section:FutureWork} discuss additional work that needs to be carried out to exploit the additional information that the new metric provides.

\section{Related Work}\label{Section:Related}

Many metrics exist for the analysis of swarms. These metrics can be categorised as \textit{Centroidal}, \textit{Inter-agent distance} and \textit{magnitude} based. The common denominator of the \textit{centroidal} and \text{inter-agent distance} techniques is that they \textit{measure} the agent distribution of a swarm based on distance. \textit{Magnitude} based analysis is based upon inter-agent interaction. 
